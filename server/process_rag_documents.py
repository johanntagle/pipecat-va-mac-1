#!/usr/bin/env python3
"""
Process markdown files and store them in the RAG database.

Usage:
    python process_rag_documents.py <company_id> <file1.md> [file2.md ...]

Example:
    python process_rag_documents.py 1 docs/menu.md docs/faq.md
"""
...
def extract_markdown_metadata(text: str, file_path: str) -> Dict[str, Any]:
    """
...
def process_markdown_file(company_id: int, file_path: str, api_key: str) -> None:
    """
    Process a single markdown file and store it in the RAG database.
    ...
    # Extract metadata
    file_metadata = extract_markdown_metadata(content, file_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process markdown files and store them in the RAG database",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python process_rag_documents.py 1 docs/menu.md
  python process_rag_documents.py 1 docs/menu.md docs/faq.md docs/policies.md
        """
    )
    parser.add_argument(
        "company_id",
    ...
    for file_path in args.files:
        ...
        if not file_path.endswith('.md'):
            logger.warning(f"File {file_path} is not a markdown file (.md)")

    # Process each file
    logger.info(f"Processing {len(args.files)} file(s)...")
    for file_path in args.files:
        process_markdown_file(args.company_id, file_path, api_key)
